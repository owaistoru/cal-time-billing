...
const END_TS = `CASE WHEN s.end_time < s.start_time THEN (s.session_date::timestamp + s.end_time + interval '1 day') ELSE (s.session_date::timestamp + s.end_time) END`;
...
router.get('/', async (req, res, next) => {
  try {
    const nameCol = await getClientsNameColumn();
    const { hasTutor, hasUser, hasNotes, hasCreated } = await flags();
    const selectCols = `s.id, s.client_id, ${nameCol ? `c.${nameCol} AS client_name` : 'NULL::text AS client_name'}, ${START_TS} AS start_time, ${END_TS} AS end_time, s.position, s.client_first_name, s.client_last_name, s.subject_code, s.course_number, s.no_show, s.status, ${hasNotes ? 's.notes' : 'NULL::text'} AS description, ${hasCreated ? 's.created_at' : 'now()'} AS created_at, ${RAW_HOURS} AS raw_hours`;
    const where = hasTutor ? 'WHERE s.tutor_id = $1' : hasUser ? 'WHERE s.user_id = $1' : '';
    const params = (hasTutor || hasUser) ? [req.user.id] : [];
    const q = await db.query(`SELECT ${selectCols} FROM sessions s ${nameCol ? 'LEFT JOIN clients c ON c.id = s.client_id' : ''} ${where} ORDER BY ${START_TS} DESC`, params);
    const rows = q.rows.map((r) => ({ ...r, hours: applyMinRules(r.position, Number(r.raw_hours)) }));
    res.json(rows);
  } catch (err) {
    next(err);
  }
...
router.post('/', validate(createSchema), async (req, res, next) => {
  try {
    ...
    if (await clientsHasUserId() && req.body.client_id) {
      const c = await db.query('SELECT id FROM clients WHERE id = $1 AND user_id = $2', [req.body.client_id, req.user.id]);
    ...
    const cols = ['client_id', 'session_date', 'start_time', 'end_time', 'position', 'client_first_name', 'client_last_name', 'subject_code', 'course_number', 'no_show', 'status'];
    const vals = [req.body.client_id ?? null, a.date, a.time, b.time, req.body.position, req.body.client_first_name ?? null, req.body.client_last_name ?? null, req.body.subject_code ?? null, req.body.course_number ?? null, !!req.body.no_show, 'submitted'];
    if (hasTutor) {
    ...
    // THE DEFINITIVE FIX IS HERE: No more "s." alias in the RETURNING clause.
    const ret = await db.query(
      `INSERT INTO sessions (${cols.join(', ')}) VALUES (${ph})
       RETURNING id, position, 
                 (session_date::timestamp + start_time) AS start_time, 
                 CASE WHEN end_time < start_time THEN (session_date::timestamp + end_time + interval '1 day') ELSE (session_date::timestamp + end_time) END AS end_time,
                 EXTRACT(EPOCH FROM (CASE WHEN end_time < start_time THEN (session_date::timestamp + end_time + interval '1 day') ELSE (session_date::timestamp + end_time) END - (session_date::timestamp + start_time))) / 3600.0 AS raw_hours`,
      vals
    );
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const where = hasTutor ? 'id = $1 AND tutor_id = $2' : hasUser ? 'id = $1 AND user_id = $2' : 'id = $1';
    const params = (hasTutor || hasUser) ? [req.params.id, req.user.id] : [req.params.id];
    const q = await db.query(`SELECT id, position, ${START_TS} AS start_time, ${END_TS} AS end_time, ${RAW_HOURS} AS raw_hours FROM sessions s WHERE ${where}`, params);
...
router.put('/:id', validate(updateSchema), async (req, res, next) => {
  try {
    ...
    const vals = [];
    let i = 1;
    for (const f of ['position', 'client_id', 'client_first_name', 'client_last_name', 'subject_code', 'course_number', 'no_show', 'status']) {
    ...
    const where = hasTutor ? `id = $${i} AND tutor_id = $${i + 1}` : hasUser ? `id = $${i} AND user_id = $${i + 1}` : `id = $${i}`;
    const q = await db.query(`UPDATE sessions s SET ${sets.join(', ')} WHERE ${where} RETURNING id, position, ${START_TS} AS start_time, ${END_TS} AS end_time, ${RAW_HOURS} AS raw_hours`, vals);
    if (!q.rowCount) return res.status(404).json({ msg: 'Not found' });
    const r = q.rows[0];
    r.hours = applyMinRules(r.position, Number(r.raw_hours));
    res.json(r);
